...
import (
	...
	"github.com/SurrealGod/jelly-shop-code/pkg/jellyauth"
...
func (u *usersUsecase) GetPassport(req *users.UserCredential) (*users.UserPassport, error) {
	...
	//access token
	accessToken, err := jellyauth.NewJellyAuth(jellyauth.Access, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})

	refreshToken, err := jellyauth.NewJellyAuth(jellyauth.Refresh, u.cfg.Jwt(), &users.UserClaims{
		Id:     user.Id,
		RoleId: user.RoleId,
	})
...
func (u *usersUsecase) RefreshPassport(req *users.UserRefreshCredential) (*users.UserPassport, error) {
	// Parse token
	claims, err := jellyauth.ParseToken(u.cfg.Jwt(), req.RefreshToken)
	if err != nil {
		return nil, err
	}
	...
	accessToken, err := jellyauth.NewJellyAuth(
		jellyauth.Access,
		u.cfg.Jwt(),
		newClaims,
	)
	...
	refreshToken := jellyauth.RepeatToken(
		u.cfg.Jwt(),
		newClaims,
